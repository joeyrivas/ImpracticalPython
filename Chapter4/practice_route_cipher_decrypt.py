...
def main():
    ...
    COLUMNS = [x for x in range(1, COLS + 1)]

    # split elements into words, not letters
    cipherlist = list(ciphertext.split())
    validate_col_row(cipherlist)
    keys = cipher_perms.perms(COLUMNS)
    for key in keys:
        print ("\nChecking key = {}".format(key))
...
def key_to_int(key):
    ...
    key_int = [int(i) for i in key_string.split()]
...
def decrypt(translation_matrix):
    """Loop through nested lists popping off last item in a string."""
    plaintext = ''
    for i in range(ROWS):
